  ...
  public Spell parseSpell(BlockPos rootPos, Transform2D transform, Glyph parentGlyph, LinkedHashSet<BlockPos> blocks, int depth) {
    ...
    if (parentGlyph == Glyph.ACTIVATE) {
      ...
      if (state.is(block)) {
        ...
        spell.connected().addAll(parseSpell(activated.relative(transform.backwards()), localTransform.withForwards(transform.forwards()), Glyph.FORWARDS, blocks, depth + 1).connected());
        return spell;
      }
